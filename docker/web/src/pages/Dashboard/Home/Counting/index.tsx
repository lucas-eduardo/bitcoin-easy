...
const ModalPurchased: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent) => {
      ...
      await api.post('purchase', { amount });
      await loadBalance();
...
const ModalSell: React.FC = () => {
  const { closeModal } = useModal();
  const { loadBalance, btcTotal } = useTransactions();

  const [amount, setAmount] = useState(0);
  const [purchaseBtc, setPurchaseBtc] = useState(0);
  const [valueFormatted, setValueFormatted] = useState('');
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent) => {
      ...
      const { data } = await api.get('btc');

      const totalReal = purchaseBtc * data.buy;

      if (amount > totalReal) {
        return toast.error('Você não possui esse saldo atualmente em bitcoin');
      }
      await api.post('sale', { amount });
      await loadBalance();
      toast.success('Bitcoin vendido com sucesso!');
      return closeModal();
    },
    [amount, closeModal, loadBalance, purchaseBtc],
  );
